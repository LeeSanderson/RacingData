...
def screen(
    ...
    importances = dict(zip(fitted, model.feature_importances_, strict=False))
...
def _print_report(result: dict[str, Any]) -> None:
    ...
    print(
        f"  Odds-derived features present: {'NO (clean)' if odds_clean else 'YES (WARNING)'}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min-importance", type=float, default=1e-6, dest="min_importance"
    )
