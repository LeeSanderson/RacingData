...
class SplitRaceTypeAlgorithm(ProxyTSRXGBoostAlgorithm):
    """ProxyTSRXGBoostAlgorithm with separate flat and jump sub-models.

    Trains distinct ProxyTSRXGBoostAlgorithm instances for flat and jump races,
    routing each prediction to the appropriate sub-model. Falls back to a model
    trained on all data when a sub-model has fewer than MIN_RACES unique races.
    """
    ...
    def __init__(self, max_horses: int = 10):
        ...
        self._flat_model = ProxyTSRXGBoostAlgorithm(max_horses=max_horses)
        self._jumps_model = ProxyTSRXGBoostAlgorithm(max_horses=max_horses)
        self._fallback_model = ProxyTSRXGBoostAlgorithm(max_horses=max_horses)
    ...
    def fit(self, train_df: pd.DataFrame) -> None:
        ...
        # Train main classifier on all data (supports AbstainWrapperAlgorithm calibration).
        super().fit(train_df)
...
class AbstainWrapperSplitAlgorithm(AbstainWrapperAlgorithm, SplitRaceTypeAlgorithm):
    """AbstainWrapperAlgorithm backed by SplitRaceTypeAlgorithm."""
