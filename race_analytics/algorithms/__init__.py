...
from race_analytics.algorithms.position_weighted_win_classifier import (
    PositionWeightedWinClassifier,
)
...
class GatedWinClassifier(GatedClassifier):
    def __init__(
        self, max_horses: int = 10, metric: str = "top_prob", coverage: float = 0.7
    ):
        super().__init__(
            WinClassifier(max_horses=max_horses), metric=metric, coverage=coverage
        )


class GatedGapWinClassifier(GatedClassifier):
    def __init__(
        self, max_horses: int = 10, metric: str = "gap", coverage: float = 0.5
    ):
        super().__init__(
            WinClassifier(max_horses=max_horses), metric=metric, coverage=coverage
        )
...
class GatedPositionWeightedWinClassifier(GatedClassifier):
    def __init__(
        ...
        super().__init__(
            PositionWeightedWinClassifier(max_horses=max_horses),
            metric=metric,
            coverage=coverage,
        )
...
class GatedRecencyWeightedWinClassifier(GatedClassifier):
    def __init__(
        ...
        super().__init__(
            RecencyWeightedWinClassifier(max_horses=max_horses),
            metric=metric,
            coverage=coverage,
        )
...
class GatedSplitDisciplineWinClassifier(GatedClassifier):
    def __init__(
        ...
        super().__init__(
            SplitDisciplineWinClassifier(max_horses=max_horses),
            metric=metric,
            coverage=coverage,
        )
...
class GatedRankingClassifier(GatedClassifier):
    def __init__(
        self, max_horses: int = 10, metric: str = "gap", coverage: float = 0.7
    ):
        super().__init__(
            RankingClassifier(max_horses=max_horses), metric=metric, coverage=coverage
        )
...
ACTIVE_ALGORITHM = ALGORITHMS[
    13
]  # GatedRecencyWeightedWinClassifier — see evaluations.md
