...
>
> **Post-capture correction (2026-06-22 — capture PRD issues 001–005).** Building the capture path
> proved one data-shape assumption here wrong: `windSurgery` is a JSON **integer** (e.g. `2`; null
> when no wind op), **not** the bool "flag" the row-6 framing implies — it is captured as `int?`.
> `trainerRtf` is likewise an integer. The genuine bools (rows 4/5/8 — headgear / gelding / jockey
> first-time) were confirmed correct and are captured as nullable bools. Rows 6 and 9 and shortlist
> item 4 below are annotated inline.
>
...
| 6 | Wind-surgery (op indicator — **int**, not bool) | `Container__WindSurgery` (`w` + `<sup>` count) · `windSurgery` | **No** — "wind op" absent from result fixtures (0) | Public DOM — `Container__WindSurgery` + `windSurgery` confirmed on live Hexham; JSON key per runner on all cards | Field universal; flag fires mainly on **jumps** (Hexham live, Warwick) + occasional flat (Brighton maiden); 0 on Yarmouth/Kempton/Gowran/HK | Clean — wind-op must be declared pre-race (BHA) | Low — `windSurgery` JSON **integer** (e.g. `2`, null = no op; **not a bool** — see correction note; DOM `Container__WindSurgery` is a fallback) | First run after a wind op is a known sharp-improvement angle, mainly NH | **go (jumps-weighted)** — cheap, high-signal-when-fires; sparse and skews jumps |
...
| 9 | Trainer form — "RTF" / win-rate | `trainerRtf` (`__NEXT_DATA__`) · trainer win-rate `<sup class="sc-25f6462b-9">` (e.g. `59%`) | **No** — current-form stat, not on result pages | Public DOM — `trainerRtf` per runner (sample 54/53/43/null/29) + win-rate `<sup>` in both live pulls | `trainerRtf` universal (all 8); win-rate `<sup>` present GB/IRE but **absent on HK** | Clean pre-race rolling stat (excludes this race); **must be frozen at capture** — recomputed daily, no historical reconstruction (currency caveat, not leakage) | Low — `trainerRtf` JSON **integer** (sample 54/53/43/null/29); win-rate `<sup>` is a brittle duplicate (avoid) | "Yard in form" is a well-supported angle — hot trainers win at elevated rates | **go (top pick)** — best plausibility×coverage at low cost; use `trainerRtf` JSON, not the `<sup>` |
