...
public partial class RacePredictor
{
    ...
    public static List<Tuple<string, double>> CalculatePermutationFeatureImportance(MLContext mlContext, IDataView trainData, ITransformer model, string labelColumnName)
    {
    ...
    public static void PlotRSquaredValues(IDataView trainData, ITransformer model, string labelColumnName, string folderPath)
    {
        ...
        var trueValues = testResults.GetColumn<float>(labelColumnName).Take(numberOfRows).ToList();

        // Get the predicted values from the test results
        var predictedValues = testResults.GetColumn<float>("Score").Take(numberOfRows).ToList();
