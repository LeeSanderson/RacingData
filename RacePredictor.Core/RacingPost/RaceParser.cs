...
{
    protected static string? GetRacePattern(string raceName)
    {
    ...
    protected static RaceType GetRaceType(string raceName, string? fences)
    {
    ...
    protected static RaceSexRestriction GetRaceSexRestriction(string raceName)
    {
    ...
    protected static (string, string) GetAgeAndRatingBands(string? bandText)
    {
        var ageBand = string.Empty;
        var ratingBand = string.Empty;
        if (!string.IsNullOrEmpty(bandText))
        {
            var bands = bandText.TrimParentheses().Split(new[] { ",", " " }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
