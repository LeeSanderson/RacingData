...
Exception: `evaluate.py` and `predict.py` are standalone Python scripts with no `.ipynb` counterpart — edit them directly.
...
```python
class Algorithm:
    def __init__(self, max_horses: int): ...
    def fit(self, train_df: DataFrame) -> None: ...
    def predict(self, races: DataFrame, horse_stats: DataFrame, jockey_stats: DataFrame) -> DataFrame: ...
