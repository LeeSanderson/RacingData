...
2. **Python (`race_analytics/`)** — installable package; notebooks build features, scripts generate predictions

**Python pipeline (in execution order):**
- `race_analytics/notebooks/FeatureAnalysis.ipynb` — encodes categoricals, calculates speed → `Data/Race_Features.csv`
- `race_analytics/notebooks/HorseStatsBuilder.ipynb` → `Data/Horse_Stats.csv`
- `race_analytics/notebooks/JockeyStatsBuilder.ipynb` → `Data/Jockey_Stats.csv`
- `race_analytics/scripts/predict.py` — reads pre-computed feature files, calls active algorithm, writes `Data/TodaysPredictions.csv`
- `race_analytics/scripts/evaluate.py` *(standalone)* — 14-fold walk-forward evaluation; reads raw `Results_YYYYMM.csv` per fold; writes nothing to disk
...
### After changes to a notebook
```powershell
python -m nbconvert --to script race_analytics/notebooks/FeatureAnalysis.ipynb
python race_analytics/notebooks/FeatureAnalysis.py
```
Replace `FeatureAnalysis` with `HorseStatsBuilder` or `JockeyStatsBuilder` as appropriate. Check console output for errors.
...
Notebooks (`.ipynb`) in `race_analytics/notebooks/` are the source of truth. The `.py` files alongside them are auto-generated by `run.ps1` via `nbconvert` and will be overwritten on the next run. **Edit only the `.ipynb` files**, never the generated `.py` files.

Exception: `race_analytics/scripts/evaluate.py` and `race_analytics/scripts/predict.py` are standalone Python scripts with no `.ipynb` counterpart — edit them directly.
...
`encode_going()` in `race_analytics/utils/data_transforms.py` defaults empty or null `Going`
