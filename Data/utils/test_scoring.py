...
def test_roi_all_correct():
    ...
    # stakes=2, winnings=7.0, profit=5.0, ROI=5/2=2.5
    preds = _predictions([(1, 10), (2, 20)])
    results = _results([
        (1, 10, 1, 3.0, "CompletedRace"),
        (2, 20, 1, 4.0, "CompletedRace"),
    ])
    assert roi(preds, results) == pytest.approx(2.5)


def test_roi_all_wrong():
    # 2 races, both wrong → winnings=0, profit=-2, ROI=-1.0
    preds = _predictions([(1, 10), (2, 20)])
    results = _results([
        (1, 10, 2, 3.0, "CompletedRace"),
        (2, 20, 3, 4.0, "CompletedRace"),
...
def test_roi_mixed():
    ...
    # stakes=2, winnings=5.0, profit=3.0, ROI=1.5
    preds = _predictions([(1, 10), (2, 20)])
    results = _results([
        (1, 10, 1, 5.0, "CompletedRace"),
        (2, 20, 2, 4.0, "CompletedRace"),
    ])
    assert roi(preds, results) == pytest.approx(1.5)


def test_roi_excludes_void_races():
    # Race 1: correct, completed (odds 4.0); Race 2: void (NonRunner)
    # Only race 1 counts: stakes=1, winnings=4.0, profit=3.0, ROI=3.0
    preds = _predictions([(1, 10), (2, 20)])
    results = _results([
        (1, 10, 1, 4.0, "CompletedRace"),
        (2, 20, 0, 3.0, "NonRunner"),
