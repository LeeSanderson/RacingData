...
def _train_row(race_id: int, horse_id: int, speed: float, last_race_speed: float) -> dict:
    return {
        ...
        "LastRaceSpeed": last_race_speed, "DaysRested": 7.0,
...
def test_regressor_full_field_winner_per_complete_race():
    ...
    horse_stats = pd.DataFrame(
        [_horse_stat(101, 15.0), _horse_stat(102, 16.0), _horse_stat(103, 17.0)]
        + [_horse_stat(201, 19.0), _horse_stat(202, 18.0), _horse_stat(203, 17.0)]
    )
    jockey_stats = pd.DataFrame([_jockey_stat(h) for h in races["HorseId"]])
