...
def test_jockey_with_no_prior_races_keeps_default(three_day_df: pd.DataFrame) -> None:
...
def test_days_since_jockey_last_raced(three_day_df: pd.DataFrame) -> None:
    CalculateJockeyStats().process_race_data(three_day_df)
    row = _paul_town_on(three_day_df, td.Chelmsford21stAt1805)
    assert row["DaysSinceJockeyLastRaced"] == 1


def test_win_percentage_with_no_wins(three_day_df: pd.DataFrame) -> None:
    CalculateJockeyStats().process_race_data(three_day_df)
    row = _paul_town_on(three_day_df, td.Chelmsford21stAt1805)
...
def test_incremental_processing_updates_prior_race_count(
